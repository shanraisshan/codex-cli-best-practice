...
>
> *† BMAD-METHOD, OpenSpec, and Matt Pocock Skills are partial Codex fits — their methodologies (installers, spec artifacts, skill workflows) are platform-agnostic, but their named slash commands and Anthropic-format `SKILL.md` files won't fire natively on Codex (no custom-command support yet, different skill loader). The pipeline names are nominal; users invoke them via prompts, manual spec edits, or by porting the skill bodies into Codex agent/prompt form.*
