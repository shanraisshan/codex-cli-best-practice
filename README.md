...
>
> *† BMAD-METHOD and OpenSpec are partial Codex fits — their installers/spec artifacts are platform-agnostic, but the named slash commands won't fire natively on Codex (no custom-command support yet). The pipeline names are nominal; users invoke them via prompts or the spec-artifact workflow.*
